...
class Scheduler:        #Scheduler class with 2 queue for type 1-2 tasks / have access to all servers/cores
    ...
    def update_servers(self):                   #calling servers handle_queue (checkout tasks in queue and assign to a core if it's idle)
...
def start(scheduler: Scheduler, tasks: List[Task]):         #system While
    ...
    while scheduler.all_accurate() or (out_tasks < MAX_TASKS and out_tasks < len(tasks)):   #repeat until 0.05 accuracy or limit (or until all finished)
